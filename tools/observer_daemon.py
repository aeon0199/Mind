...
Protocol (stdin → stdout):
    IN  : {"mode": "observe"|"stress"|"branchpoints"|"basins"|
           "propagation"|"hysteresis"|"control", ...}
...
def _build_config_hysteresis(cfg: Dict[str, Any], num_layers: int):
    ...
    return HysteresisConfig(
        ...
        backend=cfg.get("backend", "hf"),
        seed=_int_with_default(cfg.get("seed"), 42),
        perturbation_mode=str(cfg.get("perturbation_mode", "noise")),
        ...
        recovery_tokens=int(cfg.get("recovery_tokens", 32)),
        propagation_floor=float(cfg.get("propagation_floor", 1e-6)),
        temperature=float(cfg.get("temperature", 0.0)),
        top_p=float(cfg.get("top_p", 1.0)),
        top_k=int(cfg.get("top_k", 0)),
    )
...
def _build_config_basins(cfg: Dict[str, Any], num_layers: int):
    ...
    prompt_key = str(cfg.get("prompt_key", "custom"))
    if prompt_key == "custom":
        prompt = str(cfg.get("prompt") or "").strip()
        spec = build_generic_prompt_spec(
            prompt,
            key="custom",
            prompt_class=str(cfg.get("prompt_class", "custom")),
        )
    else:
        spec = get_prompt_spec(prompt_key)
        provided_prompt = cfg.get("prompt")
        if provided_prompt is not None and str(provided_prompt) != spec.prompt:
            raise ValueError(
                f"prompt does not match registered basin prompt {prompt_key!r}"
            )
    layer = resolve_semantic_layer(cfg.get("layer", "late"), num_layers)
    return BasinConfig(
        prompt=spec.prompt,
        prompt_key=spec.key,
        ...
        continuation_tokens=_int_with_default(
            cfg.get("continuation_tokens"),
            48,
        ),
        max_episodes=_int_with_default(cfg.get("max_episodes"), 3),
        backend=cfg.get("backend", "hf"),
        seed=_int_with_default(cfg.get("seed"), 42),
        intervention_layer=int(layer),
        intervention_type=str(cfg.get("intervention_type", "additive")),
        intervention_magnitude=float(cfg.get("magnitude", 0.30)),
        intervention_magnitude_relative=(
            not cfg.get("absolute_magnitude", False)
        ),
        intervention_seed=int(cfg.get("intervention_seed", 42)),
        ...
        tie_tolerance=float(cfg.get("tie_tolerance", 0.025)),
        with_diagnostics=bool(cfg.get("with_diagnostics", True)),
        temperature=float(cfg.get("temperature", 0.0)),
        top_p=float(cfg.get("top_p", 1.0)),
        top_k=int(cfg.get("top_k", 0)),
...
def _run_request(request: Dict[str, Any], backend) -> Dict[str, Any]:
    ...
    if mode == "basins":
        ...
        cfg = _build_config_basins(cfg_values, num_layers)
        probe_layers = resolve_probe_layers(
            cfg_values.get("probe_layers", "auto"),
            num_layers,
        )
