#!/usr/bin/env python3
"""Historical controller-pair analyzer with invalid shifted/cascading labels."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--acknowledge-invalid-labels",
        action="store_true",
        help="Run the historical shifted/cascading-label analysis despite its invalidity.",
    )
...
def main() -> None:
    ...
    if not args.acknowledge_invalid_labels:
        raise SystemExit(
            "Refusing to run: this historical analyzer treats downstream controller-run "
            "mismatches as branchpoint labels. Those labels are shifted and cascading, "
            "so they are invalid for local flippability. Pass "
            "--acknowledge-invalid-labels only for historical audit work."
        )
